...
% phi is KxW matrix                topic*word
% theta is KxD matrix              topic*document
% ALPHA and BETA are hyperparameters

phi = normalise(phi + BETA, 2);
theta = normalise(theta + ALPHA, 1);
[wi, di, xi] = find(WD); %wi row word index,di column document index,xi word counts
...
if nargin < 2
    z = sum(A(:));
    % Set any zeros to one before dividing, or diving cannot be calculated
